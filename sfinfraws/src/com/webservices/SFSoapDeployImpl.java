...
public class SFSoapDeployImpl implements NotificationPort {
	...
	public boolean notifications(String organizationId, String actionId,
		...
		for (int i = 0; i < arrSize; i++) {
			...
			System.out.println("metadata Log Id: "+ metadataLogId + "  -Org Id: "+tOrgId);
			try {
				ForceDepService deploymentService = new ForceDepService();	
				if ((tOrgId != null && !tOrgId.isEmpty())
						&& (metadataLogId != null && !metadataLogId.isEmpty())) {
					deploymentService.deploy(bOrgId, bOrgToken, bOrgURL, bOrgRefreshToken, metadataLogId);

					
					//deploymentService.deploy(bOrgToken, metadataLogId);
				}
			} catch (Exception e) {
				e.printStackTrace();
				return false;
